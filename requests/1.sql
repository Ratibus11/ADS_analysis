...
FROM (
	SELECT
		...
		ROW_NUMBER() OVER (
			...
			ORDER BY quantite DESC) AS top_vente,
		quantite,
		dieu.nom AS dieu
	FROM (
		SELECT SUM(quantite) AS quantite, id_dieu, id_province
		FROM (
			SELECT quantite, id_dieu, id_province
			FROM (
				SELECT id_province, quantite, id_dieu
				FROM decoration
	...
	ORDER BY province ASC, quantite DESC)
